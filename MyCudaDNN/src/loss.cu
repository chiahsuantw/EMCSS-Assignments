...
[[maybe_unused]] __device__ float clip(float prediction, float epsilon = 1e-12) {
    return fmin(fmax(prediction, epsilon), 1.f - epsilon);
}

__global__ void
softmax_loss_kernel(float *reduced_loss, float *predict, const float *target, float *workspace, int batch_size,
                    int num_outputs) {
...
float CrossEntropyLoss::loss(Tensor<float> *predict, Tensor<float> *target) {
    ...
    cudaOccupancyMaxActiveBlocksPerMultiprocessor(&num_blocks_per_sm, softmax_loss_kernel, BLOCK_DIM_1D,
